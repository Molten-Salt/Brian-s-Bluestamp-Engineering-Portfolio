...
A stationary solar panel only points directly at the sun for a moment each day, which means it spends most of its time collecting light at an angle instead of head-on. To close that gap, I built a dual-axis solar tracker: a mount that continuously re-aims the panel so its surface stays perpendicular to the sun as it moves across the sky, squeezing more usable sunlight — and therefore more electricity — out of the same panel. I also added a liquid cooling loop, since panel efficiency drops as temperature rises. Details on how both systems work are under the milestones below.
...
I installed the photoresistors to complete the solar tracker. Soldering all these parts onto a small perfboard came with plenty of difficulties — I created a number of shorts, which meant a lot of desoldering and re-work. Along the way I learned CAD and soldering, and I plan to keep building engineering projects on my own.

This milestone also completed the cooling system. Solar panels lose efficiency as their temperature climbs above about 25°C, so I paired the tracker with a negative-feedback liquid cooling loop to keep the panel closer to that optimum. A temperature sensor on the back of the panel feeds the Arduino a continuous reading; once it drifts a set amount above the target threshold, the Arduino switches on a pump (through a MOSFET, since the pump draws more current than a digital pin can safely supply) that pushes cooled water into an aluminum cooling block mounted against the panel. That block is machined with a dense network of microscopic internal channels, which dramatically increases the surface area in contact with the water and speeds up heat transfer from the metal into the coolant. The now-warmed water is pumped out to a radiator, where a fan blows air across the fins to carry the heat away, before the cooled water returns to the pump's reservoir to begin the cycle again — a closed loop that keeps working for as long as the panel stays above the threshold, and shuts off automatically once it cools back down.
...
The full tracker scales this same idea up to four light-dependent resistors (LDRs), one mounted near each corner of the panel, to sense the direction of brightest light. Each LDR's resistance drops as more light hits it, so shining light unevenly across the four sensors produces four different voltage readings when the Arduino samples them on its analog pins. The code averages the readings pairwise — top vs. bottom, and left vs. right — to get one brightness differential for tilt and one for pan. If the top sensors are reading brighter than the bottom (or the left brighter than the right) by more than a small deadband, the Arduino nudges the corresponding servo a few degrees in that direction; if the readings are already balanced, it holds position. Running this comparison in a continuous loop lets the mount creep toward the sun in small steps throughout the day, on both the vertical (tilt) and horizontal (pan) axes, rather than jumping straight to a calculated angle. The deadband keeps the servos from jittering back and forth chasing tiny, insignificant differences between sensors, and software limits on each axis keep the panel from over-rotating and straining the mount.
