...
To maximize electricity generation, I built a device that tracks the sun. By comparing the voltage difference between two photoresistors on opposite sides of the panel, an Arduino commands a servo motor to rotate, dynamically moving the solar panel along two axes so that it stays perpendicular to the sun.

Because solar panels lose efficiency as temperature rises above 25°C, I also built a negative-feedback cooling system. When the panel's temperature rises a set amount above a target threshold, a pump pushes cooled water into an aluminum cooling block. The block's microscopic channels maximize surface area (and therefore collisions with the metal), transferring heat from the panel into the water. That water then flows to a radiator, where a fan helps dissipate the heat, before returning to the pump's reservoir to be recirculated.
